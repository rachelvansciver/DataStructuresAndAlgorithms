...
public class MyHashMap<K, V> implements MyMap<K, V> {
    ...
    public LinkedList<V> getAll(K key){
        ...
        //holds values with the same key
        int bucketIndex = hash(key.hashCode());
        if(table[bucketIndex] != null){
            LinkedList<Entry<K, V>> bucket = table[bucketIndex];
            for(Entry<K, V> entry: bucket)
    ...
    public V put(K key, V value) {
        if (get(key) != null) { // The key is already in the map
            int bucketIndex = hash(key.hashCode());
            LinkedList<Entry<K, V>> bucket = table[bucketIndex];
            /**hashmap would normally write the new value to the key and drop
             * the reference to the old one, which would be collected by the
             * garbage collector, but in this case it does nothing, which means duplicates
             * can be added.
             */
        }
